...
func exploreScreen(win fyne.Window) fyne.CanvasObject {
	/*
	...
	*/
	...
	//var displayedpostslist * widget.List
	//var displayedposts [] string//RegistredNameInfo

	displayedposts := binding.BindStringList(
		&[]string{},
	)
	
	fmt.Printf("%v",displayedposts)
	displayedpostslist := widget.NewListWithData(displayedposts,
		...
		go func() {
			for {
				//fmt.Println("*******!!!!!!!!",displayedposts)
				if daemon.Mn!=nil{
					displayedposts.Set(daemon.Mn.GetPosts(30))
	...
	displayedpostslistcontainer:=container.New(layout.NewGridWrapLayout(fyne.NewSize(appscreenWidth, appscreenHeight*3/4)),displayedpostslist)
	//layout:=container.NewVBox(displayedpostslistcontainer,nameregistrationbuttoncontainer)
	return displayedpostslistcontainer
