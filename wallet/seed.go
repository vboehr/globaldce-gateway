...
func GenerateMnemonicSeedString() (string, error) {
	...
	/*
		wordList := []string{
			...
			"flower", "guitar", "hat", "ice cream", "jungle",
		}*/
	seed := ""
	for i := 0; i < 15; i++ {
		// Generate a random index within the word list range
		index, err := rand.Int(rand.Reader, big.NewInt(int64(len(wordList))))
		if err != nil {
			return "", err
		}

		// Append the randomly selected word to the seed string
		seed += wordList[index.Int64()] + " "
	}
